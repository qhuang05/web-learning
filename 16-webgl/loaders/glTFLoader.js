...
THREE.glTFLoader.prototype.load = function (url, callback) {
  ...
  function convertAxisAngleToQuaternion(rotations, count) {
    ...
    for (i = 0; i < count; i++) {
      axis
        .set(rotations[i * 4], rotations[i * 4 + 1], rotations[i * 4 + 2])
        .normalize();
  ...
  ClassicGeometry.prototype.buildBufferGeometry = function () {
    ...
    geometry.addAttribute(
      "index",
      new THREE.BufferAttribute(this.indexArray, 1)
    );
  ...
  VertexAttributeDelegate.prototype.arrayResourceAvailable = function (
    glResource,
    ctx
  ) {
    ...
    if (semantic == "POSITION") {
      ...
      floatArray = new Float32Array(
        glResource,
        0,
        attribute.count * componentsPerElementForGLType(attribute.type)
      );
      for (i = 0, l = floatArray.length; i < l; i += 3) {
        geom.geometry.vertices.push(
          new THREE.Vector3(floatArray[i], floatArray[i + 1], floatArray[i + 2])
        );
      }
      ...
      for (i = 0, l = floatArray.length; i < l; i += 2) {
        geom.uvs.push(
          new THREE.Vector2(floatArray[i], 1.0 - floatArray[i + 1])
        );
      }
      ...
      floatArray = new Float32Array(
        glResource,
        0,
        attribute.count * nComponents
      );
      for (i = 0, l = floatArray.length; i < l; i += 4) {
        geom.geometry.skinWeights.push(
          new THREE.Vector4(
            floatArray[i],
            floatArray[i + 1],
            floatArray[i + 2],
      ...
      for (i = 0, l = floatArray.length; i < l; i += 4) {
        geom.geometry.skinIndices.push(
          new THREE.Vector4(
            floatArray[i],
            floatArray[i + 1],
            floatArray[i + 2],
  ...
  VertexAttributeDelegate.prototype.bufferResourceAvailable = function (
    glResource,
    ctx
  ) {
    var geom = ctx.geometry;
    ...
    if (semantic == "POSITION") {
      ...
      geom.geometry.addAttribute(
        "position",
        new THREE.BufferAttribute(floatArray, 3)
      );
    } else if (semantic == "NORMAL") {
      floatArray = new Float32Array(
        glResource,
      ...
      geom.geometry.addAttribute(
        "uv",
        new THREE.BufferAttribute(floatArray, nComponents)
      );
    } else if (semantic == "WEIGHT") {
      nComponents = componentsPerElementForGLType(attribute.type);
      floatArray = new Float32Array(
  ...
  VertexAttributeDelegate.prototype.resourceAvailable = function (
    glResource,
    ctx
  ) {
  ...
  Mesh.prototype.checkComplete = function () {
    var self = this;
    if (
      this.onCompleteCallbacks.length &&
      this.primitives.length == this.loadedGeometry
    ) {
  ...
  AnimationParameterDelegate.prototype.handleError = function (
    errorCode,
    info
  ) {
    // FIXME: report error
  ...
  AnimationParameterDelegate.prototype.convert = function (resource, ctx) {
    ...
    switch (parameter.type) {
      ...
      case WebGLRenderingContext.FLOAT_VEC3:
      case WebGLRenderingContext.FLOAT_VEC4:
        glResource = new Float32Array(
          resource,
          0,
          parameter.count * componentsPerElementForGLType(parameter.type)
        );
        break;
      default:
        break;
  ...
  AnimationParameterDelegate.prototype.resourceAvailable = function (
    glResource,
    ctx
  ) {
  ...
  InverseBindMatricesDelegate.prototype.handleError = function (
    errorCode,
    info
  ) {
    // FIXME: report error
  ...
  InverseBindMatricesDelegate.prototype.resourceAvailable = function (
    glResource,
    ctx
  ) {
    var skin = ctx.skin;
  ...
  var ThreeGLTFLoader = Object.create(glTFParser, {
      ...
      value: function (material) {
        ...
        if (!fragmentShader) {
          console.log(
            ...
            material.params.fragmentShader
          );
      ...
      value: function (materialId, technique, values, params) {
        ...
        if (diffuse) {
          ...
          if (texture) {
            ...
            if (textureEntry) {
              {
                var imageEntry = this.resources.getEntry(
                  textureEntry.description.source
                );
                if (imageEntry) {
                  texturePath = imageEntry.description.path;
                }

                var samplerEntry = this.resources.getEntry(
                  textureEntry.description.sampler
                );
                if (samplerEntry) {
        ...
        if (
          !(values.ambient === undefined) &&
          !(typeof values.ambient == "string")
        ) {
    ...
    handleMaterial: {
      value: function (entryID, description, userInfo) {
        ...
        var technique = this.resources.getEntry(
          description.instanceTechnique.technique
        );
        var materialParams = {};
        var values = description.instanceTechnique.values;

        var materialType = this.threeJSMaterialType(
          entryID,
          technique,
          values,
          materialParams
    ...
    handleMesh: {
      value: function (entryID, description, userInfo) {
        ...
        for (var i = 0; i < primitivesDescription.length; i++) {
          ...
          if (
            primitiveDescription.primitive === WebGLRenderingContext.TRIANGLES
          ) {
            var geometry = new ClassicGeometry();
            var materialEntry = this.resources.getEntry(
            ...
            var bufferEntry = this.resources.getEntry(
              indices.description.bufferView
            );
            var indicesObject = {
            ...
            var alreadyProcessedIndices = THREE.GLTFLoaderUtils.getBuffer(
              indicesObject,
              indicesDelegate,
              indicesContext
            );
            ...
            allAttributes.forEach(function (semantic) {
              ...
              var attribContext = new VertexAttributeContext(
                attributeObject,
                semantic,
                geometry
              );

              var alreadyProcessedAttribute = THREE.GLTFLoaderUtils.getBuffer(
                attributeObject,
    ...
    handleNode: {
      value: function (entryID, description, userInfo) {
        ...
        if (description.instanceSkin) {
          var skinEntry = this.resources.getEntry(
              ...
              for (i = 0; i < len; i++) {
                ...
                mesh.primitives.forEach(function (primitive) {
                  ...
                  if (threeMesh && dobones) {
                    ...
                    threeMesh.bind(
                      new THREE.Skeleton(bones, boneInverses, false)
                    );
                  }
    ...
    buildSkins: {
      value: function (node) {
        if (node.instanceSkin) this.buildSkin(node);
    ...
    handleScene: {
      value: function (entryID, description, userInfo) {
        if (!description.nodes) {
          console.log(
            "ERROR: invalid file required nodes property is missing from scene"
          );
          return false;
        }
    ...
    handleAnimation: {
      value: function (entryID, description, userInfo) {
        ...
        params.forEach(function (param) {
          ...
          var paramContext = new AnimationParameterContext(
            paramObject,
            animation
          );
    ...
    handleSkin: {
      value: function (entryID, description, userInfo) {
        ...
        skin.inverseBindMatricesDescription.id =
          entryID + "_inverseBindMatrices";

        var bufferEntry = this.resources.getEntry(
          inverseBindMatricesDescription.bufferView
        );
        ...
        var bufferView = this.resources.getEntry(
